<template>
  <div>
    <div v-if="$page.props.flash.success && show" class="flex items-center justify-between mb-8 max-w-3xl mx-auto bg-green-500 rounded">
      <div class="flex items-center">
        ...
        <div class="py-4 text-white text-sm font-medium" v-html="$page.props.flash.success"> </div>
      </div>
    ...
    <div v-if="($page.props.flash.error || Object.keys($page.props.errors).length > 0) && show" class="flex items-center justify-between mb-8 max-w-3xl mx-auto bg-red-500 rounded">
      <div class="flex items-center">
        ...
        <div v-if="$page.props.flash.error" class="py-4 text-white text-sm font-medium"  v-html="$page.props.flash.error"> </div>
        <div v-else class="py-4 text-white text-sm font-medium">
          <span v-if="Object.keys($page.props.errors).length === 1">Hay un error de formulario.</span>
          <span v-else>Hay {{ Object.keys($page.props.errors).length }} errores de formaulario.</span>
        </div>
      </div>
      <button type="button" class="group mr-2 p-2" @click="show = false">
